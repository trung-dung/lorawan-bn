...
// ----------------------------------------------------------------
// Serial Implementation -- UART 5 - STM32L476RGT6
// ----------------------------------------------------------------

GPSSerial::GPSSerial(int rxSize): SerialPipe(rxSize)
{
...
{
	HW_UART5_Init();
	GpioInit(&GpsReset, GPS_RST_N, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0); 
	HAL_Delay(50);
	GpioWrite(&GpsReset, 1);
	return true;
}
...
{ 
    HW_UART5_Send((const char *) buf, len);
	return 0;
}
