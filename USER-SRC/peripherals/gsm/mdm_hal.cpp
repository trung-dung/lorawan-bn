...
bool MDMParser::_powerOn(void)
{
...
#else
		GpioWrite(&Mdm_rts, 0); // VERY IMPORTANT FOR CORRECT OPERATION W/O HW FLOW CONTROL!!
#endif
		GpioWrite(&Mdm_dtr, 0);
    ...
    while (i--) {
        ...
        GpioWrite(&Mdm_pwr, 0); HAL_Delay_ms(50);
        GpioWrite(&Mdm_pwr, 1); HAL_Delay_ms(10);

        // SARA-G35 >5ms, LISA-C2 > 150ms, LEON-G2 >5ms
        GpioWrite(&Mdm_pwr, 0); HAL_Delay_ms(150);
        GpioWrite(&Mdm_pwr, 1); HAL_Delay_ms(100);
...
// Section: Functionality of SMS services
//// ----------------------------------------------------------------
//
...
//
// Section: Functionality of USSD services
//----------------------------------------------------------------
...
// Section: Debug and print system status include Device information and Network information + IP of socket
// ----------------------------------------------------------------
...
MDM_DeviceSerial::MDM_DeviceSerial(int rxSize /*= 256*/) :
    SerialPipe(rxSize)
{
...
{
		HW_UART2_Init();
		GpioInit(&Mdm_dtr, MDM_DTR, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0); 
		GpioInit(&Mdm_pwr, MDM_PWR, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0); 
		GpioInit(&Mdm_rst, MDM_RESET, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0);
		GpioInit(&Mdm_ri, MDM_RI, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0);	
		GpioInit(&Mdm_rts, MDM_RTS, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0);	
}

int MDM_DeviceSerial::_send(const void* buf, int len)
{
    HW_UART2_Send((const char*)buf, len);
    return 0;
}
