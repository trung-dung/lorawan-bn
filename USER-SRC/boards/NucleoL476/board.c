 ...
 * 
 * modified and maintained by Kien Hoang ( Sanslab - 5/2019)
 */
#include "stm32l4xx.h"
#include "utilities.h"
#include "gpio.h"
...
/*!
 * UART3 FIFO buffers size
 */
#define UART3_FIFO_RX_SIZE                                512
#define GPS_FIFO_SIZE                                     1024

uint8_t Uart3RxBuffer[UART3_FIFO_RX_SIZE];

uint8_t GPSRxBuffer[GPS_FIFO_SIZE];
...
void BoardInitMcu( void )
{
    if( McuInitialized == false )
    {
		...
		// init fifo to receive data from UART3
		FifoInit( &fifoDataRx, Uart3RxBuffer, UART3_FIFO_RX_SIZE );

		// Init fifo for GPS
		FifoInit( &fifoGpsRx, GPSRxBuffer, GPS_FIFO_SIZE );
...
{	
    ...
    HAL_SYSTICK_Config( HAL_RCC_GetHCLKFreq( ) / 1000 ); // mix config
...
void assert_failed( uint8_t* file, uint32_t line )
{
    ...
    DEBUG_D( "Wrong parameters value: file %s on line %lu\r\n", ( const char* )file, line );
    /* Infinite loop */
    while( 1 )
    {
    }
