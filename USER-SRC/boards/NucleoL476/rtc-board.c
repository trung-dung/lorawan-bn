...
// rtc clock set newtim

uint8_t RtcSetNewCalenarValue(uint8_t HH, uint8_t MIN, uint8_t SEC, uint8_t DD, uint8_t MM, uint16_t YYYY)
{
...
uint8_t RtcSetNewTimeValue(uint8_t HH, uint8_t MIN, uint8_t SEC)
{
	RTC_TimeTypeDef sTime;
	...
	sTime.Minutes = MIN;
	sTime.Seconds = SEC;
	if (HAL_RTC_SetTime(&RtcHandle, &sTime, RTC_FORMAT_BIN) == HAL_OK) return 1;
	else return 0;	
}

uint8_t RtcSetNewDateValue(uint8_t DD, uint8_t MM, uint16_t YYYY)
{
	RTC_DateTypeDef sDate;	
	/* Check year and month */
	uint8_t YY = YYYY - 2000;
	...
	sDate.Date = DD;
	sDate.Year = (YYYY - 2000);
	if(HAL_RTC_SetDate(&RtcHandle, &sDate, RTC_FORMAT_BIN) == HAL_OK) return 1;
	else return 1;
}
