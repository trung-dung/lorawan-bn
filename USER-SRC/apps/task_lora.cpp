 ...
 */
#define APP_TX_DUTYCYCLE                            30000 // 30 [s]
...
void vtaskLoRa(void *params)
{
    ...
    DEBUG_D( "###### ===== LoRaWAN V1.0.3 Class A ==== ######\r\n\r\n" );
    ...
    while( 1 )
    {
        ...
        switch( DeviceState )
        {
            ...
            {
                DEBUG_D( "DevEui      : %02X", DevEui[0] );
                for( int i = 1; i < 8; i++ )
                {
                    DEBUG_D( "-%02X", DevEui[i] );
                }
                DEBUG_D( "\r\n" );
                DEBUG_D( "AppEui      : %02X", JoinEui[0] );
                for( int i = 1; i < 8; i++ )
                {
                    DEBUG_D( "-%02X", JoinEui[i] );
                }
                DEBUG_D( "\r\n" );
                DEBUG_D( "AppKey      : %02X", NwkKey[0] );
                for( int i = 1; i < 16; i++ )
                {
                    DEBUG_D( " %02X", NwkKey[i] );
                }
                DEBUG_D( "\n\r\n" );
#if( OVER_THE_AIR_ACTIVATION == 0 )
                DEBUG_D( "###### ===== JOINED ==== ######\r\n" );
                DEBUG_D( "\r\nABP\r\n\r\n" );
                DEBUG_D( "DevAddr     : %08lX\r\n", DevAddr );
                DEBUG_D( "NwkSKey     : %02X", FNwkSIntKey[0] );
                for( int i = 1; i < 16; i++ )
                {
                    DEBUG_D( " %02X", FNwkSIntKey[i] );
                }
                DEBUG_D( "\r\n" );
                DEBUG_D( "AppSKey     : %02X", AppSKey[0] );
                for( int i = 1; i < 16; i++ )
                {
                    DEBUG_D( " %02X", AppSKey[i] );
                }
                DEBUG_D( "\n\r\n" );
...
static void PrepareTxFrame( uint8_t port )
{
    ...
    case 2:
        {
			CreateDataPackage(0, 0, 0);
            AppDataSizeBackup = AppDataSize = 41;
			for(uint8_t i = 0; i < 41; i++) AppDataBuffer[i] = DATA_PACKET[i];
        }
        break;
    case 224:
...
static bool SendFrame( void )
{
    ...
    else
    {
        ...
        else
        {
            ...
            mcpsReq.Req.Confirmed.Datarate = (uint8_t) LORAWAN_DEFAULT_DATARATE;
        }
    }
...
#endif
        {
            DEBUG_D("%04X ", mibGet.Param.ChannelsMask[i] );
        }
        DEBUG_D("\r\n");
    }
